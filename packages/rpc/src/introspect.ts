import { Class, ContainerType, InternalError, assert, string } from '@untype/toolbox';
...
const introspectClassKey = ([name, value]: [name: string, value: unknown]) => {
    const path = string.trimToNull(name.toLocaleLowerCase());
    ...
    if (value.type === 'REST') {
        ...
        // - ['GET /users']
        // - ['POST /users']
        const parts = path.split(' ').filter(assert.exists);
