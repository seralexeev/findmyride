...
const getAllMigrations = async (directory: string) => {
    ...
    const migrations = files
        .map((x) => {
            ...
            const [idStr, ...rest] = fileName.split('_');
...
const migrationRunner = `import { ConsoleLogger, LoggerType } from '@untype/toolbox';
...
export class Migrations {
    ...
    public static apply = async () => {
        ...
        await new Migrations(ConsoleLogger.Instance, pg).run();
