...
export class Pg implements PoolWrapper {
    ...
    public constructor(master: PoolOrConfig, options?: { applicationName?: string; replicas?: PoolOrConfig[] }) {
        const { applicationName, replicas = [] } = options ?? {};

        this.master = new PooledPg(this, { applicationName, pool: master });
        this.replicas = replicas.map((pool) => new PooledPg(this, { applicationName, pool }));
...
class TransactionScope implements Transaction {
    ...
    public constructor(
        private pool: PooledPg,
        private options?: { isolationLevel?: IsolationLevel },
    ) {}
