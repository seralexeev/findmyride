...
describe('Pg readonly replicas', () => {
    ...
    beforeAll(async () => {
        container = await new GenericContainer('ghcr.io/baosystems/postgis:16-3.4')
            .withExposedPorts(5432)
            .withEnvironment({ POSTGRES_USER: 'untype', POSTGRES_PASSWORD: 'untype', POSTGRES_DB: 'untype' })
            .start();

        const temp = new Pg(`postgres://untype:untype@${container.getHost()}:${container.getMappedPort(5432)}/untype`);
        ...
        pg = new Pg(`postgres://untype:untype@${container.getHost()}:${container.getMappedPort(5432)}/untype`, {
            replicas: [
