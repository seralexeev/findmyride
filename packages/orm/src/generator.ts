...
const generate = async (
    ...
    while (queue.length > 0) {
        ...
        const { content, importEntities, accessorContent } = processType(item, {
            schema,
        ...
        if (!dryRun) {
            ...
            if (accessorContent) {
                const dirExists = await fileExists(`${directory}/accessors`);
                if (!dirExists) {
                    await fs.mkdir(`${directory}/accessors`, { recursive: true });
                }

                const filename = `${directory}/accessors/${entityName}Accessor.ts`;
                const exists = await fileExists(filename);
                if (!exists) {
                    await fs.writeFile(filename, accessorContent);
                }
            }
        } else {
...
const processType = (
    ...
    if (createField) {
        content += `import { ${entityName}Accessor } from '../accessors/${entityName}Accessor';`;
    }
    ...
    if (createField) {
        ...
        content += `\n`;
        content += `export const ${entityName} = new ${entityName}Accessor();`;
        content += `\n`;

        accessorContent += `import { EntityAccessor } from '@untype/orm';\n`;
        accessorContent += `import type { ${entityName} } from '../generated/${entityName}';\n`;
        accessorContent += `\n`;
        accessorContent += `export class ${entityName}Accessor extends EntityAccessor<${entityName}> {\n`;
        accessorContent += `    public constructor() {\n`;
        accessorContent += `        super('${entityName}'${maybeSchema});\n`;
        accessorContent += `    }\n`;
        accessorContent += `}\n`;
    }

    return { content, importEntities, accessorContent };
};
...
const getPrimaryKeys = (schema: GraphQLSchema, entityNames: string[]) => {
    ...
    return Object.entries(fields)
        ...
        .reduce<Record<string, Partial<Record<string, string>>>>((acc, [name, field]) => {
            acc[name.toLocaleLowerCase()] = array.reduceBy(
                field.args,
                ({ name }) => name,
                ({ type }) => {
                    if (isNonNullType(type)) {
