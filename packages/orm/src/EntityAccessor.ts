...
import { NotFoundError } from '@untype/toolbox';
...
export class EntityAccessor<T> {
    ...
    public createMapper = <S extends FieldSelector<T, S>, R>(selector: S, map: (item: SelectorShape<T, S>) => R) => {
        return {
            selector,
            map,
            mapList: (items: Array<SelectorShape<T, S>>) => items.map(map),
        };
    };
